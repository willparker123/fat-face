...
# =============================================================================
        if ord == np.inf:
            ...
            #loss = loss_fn(outputs, yvar) + penalty * torch.sum(torch.abs(delta))
            loss0 = loss_fn(outputs, yvar)
...
class PGDAttack(Attack, LabelMixin):
    ...
    def __init__(
            ...
            ord=np.inf, targeted=False, penalty=1, mad=[1,1]):
        """
...
class L1PGDAttack(PGDAttack):
    def __init__(
            ...
            targeted=False, penalty=1, mad=[1, 1]):
        ord = 1
        super(L1PGDAttack, self).__init__(
            predict, loss_fn, eps, nb_iter, eps_iter, rand_init,
            clip_min, clip_max, ord, targeted, penalty, mad)
